...
class RerankerService:
    ...
    def _create_reranked_doc(self, original_doc: DocumentSearchResultOut, rerank_score: float, final_score: float, rerank_method: str) -> DocumentSearchResultOut:
        """创建一个新的重排序文档对象"""
    ...
    def _load_cross_encoder(self):
        ...
        try:
            ...
            self.cross_encoder = CrossEncoder(
                f'cross-encoder/{self.reranker_model_name}',
                device=device,
...
只返回分数，不要解释。"""
    ...
    def _rerank_with_cross_encoder(self, query: str, documents: List[DocumentSearchResultOut]) -> List[DocumentSearchResultOut]:
        """使用交叉编码器进行重排序"""
        # 尝试加载交叉编码器
    ...
    def _rerank_with_keyword_boost(self, query: str, documents: List[DocumentSearchResultOut]) -> List[DocumentSearchResultOut]:
        """基于关键词匹配进行重排序提升"""
        try:
