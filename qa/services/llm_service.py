...
class LLMService:
    ...
    def generate_response(
        self, query: str, context: str, conversation_history: list[dict[str, str]] | None = None
    ) -> dict[str, Any]:
        """
        调用千问大模型生成回答
    ...
    def generate_response_stream(
        self, query: str, context: str, conversation_history: List[Dict[str, str]] = None, model: str | None = None
    ) -> Generator:
        """
        流式生成回答，边生成边返回
        ...
        try:
            ...
            system_prompt = """你是一个智能助手，可以根据提供的文档和对话历史回答问题。
                                ...
                                2. 如果问题需要查找具体的文档知识，使用提供的上下文信息回答
                                3. 可以结合对话历史和文档上下文提供更完整的回答
                                4. 只有在对话历史和文档上下文都完全没有相关信息时，才说不知道
                    ...
                    if hasattr(chunk.output, "choices") and len(chunk.output.choices) > 0:
                        ...
                        if hasattr(choice, "message") and hasattr(choice.message, "content"):
                            ...
                            if content and len(content) > len(full_content):
                                ...
                                result = {"answer_delta": delta, "finished": False, "model": model_to_use}
            ...
            # 生成结束
            yield {"answer_delta": "", "finished": True, "model": model_to_use}
    ...
    @staticmethod
    def generate_response_static(
        query: str, context: str, conversation_history: list[dict[str, str]] | None = None
    ) -> str:
        """静态方法版本，适用于简单调用"""
    ...
    def get_llm_instance(self) -> LLMProtocol:
        ...
        try:
            ...
            class QwenLLMWrapper:
                def __call__(self, prompt: str) -> str:
                    response = Generation.call(model="qwen-turbo", prompt=prompt, max_tokens=500, temperature=0.3)
                    if response.status_code == 200:
