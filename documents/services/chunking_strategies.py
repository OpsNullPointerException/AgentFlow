...
class SemanticChunkingStrategy(ChunkingStrategy):
    ...
    def _extract_document_structure(self, text: str) -> Dict[str, Any]:
        ...
        # 识别中文章节
        chinese_header_pattern = re.compile(r"^第([一二三四五六七八九十百千万]+)[章节篇]\s*[:：]?\s*(.+)$")
...
class LocalModelChunkingStrategy(ChunkingStrategy):
    ...
    def _find_best_breakpoint(
        self, text: str, start_idx: int, chunk_size: int, potential_breakpoints: List[int]
    ) -> int:
        """使用本地模型找到最佳分块点"""
        ...
        candidate_breakpoints = [
            bp for bp in potential_breakpoints if bp > start_idx and abs(bp - target_end) < chunk_size / 2
        ]
    ...
    def _evaluate_breakpoints_with_model(
        self, text: str, start_idx: int, candidate_breakpoints: List[int], chunk_size: int = None
    ) -> int:
        """使用本地模型评估候选断点的语义完整性"""
        try:
            import torch
            ...
            with torch.no_grad():
                for segment in segments:
                    inputs = self.tokenizer(segment, return_tensors="pt", truncation=True, max_length=512).to(
                        self.device
                    )
